                ...
                >
                  <div className="flex items-center gap-3">
                    <span className={`transition-transform group-hover:scale-110 ${
                      activeItem === item.label && !item.hasSubmenu ? 'text-white' : 'text-gray-600 group-hover:text-[#69569F]'
                    }`}>
                      {item.icon}
                    </span>
                  ...
                  {item.hasSubmenu && (
                    <span className={`transition-transform ${usersOpen ? 'rotate-180' : ''} ${
                      activeItem === item.label && !item.hasSubmenu ? 'text-white' : 'text-gray-500 group-hover:text-[#69569F]'
                    }`}>
                ...
                {item.hasSubmenu && usersOpen && (
                  <div className="ml-6 mt-2 space-y-1 border-l-2 border-gray-200 pl-2">
                    {item.submenu?.map((subItem, subIndex) => (
                      ...
                      >
                        <span className={`transition-transform group-hover:scale-110 ${
                          activeItem === subItem.label ? 'text-white' : 'text-gray-500 group-hover:text-[#69569F]'
                        }`}>
                          {subItem.icon}
                        </span>
