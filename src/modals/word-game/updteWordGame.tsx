...
interface UpdateWordGameModalProps {
    open: boolean;
    onClose: () => void;
    onSuccess: () => void;
...
export const UpdateWordGameModal: React.FC<UpdateWordGameModalProps> = ({
    open,
    onClose,
    onSuccess,
    categories,
    ...
    useEffect(() => {
        if (game) {
            ...
            setFormData({
                ...
                translations: [
                    { title: game.translations.find(t => t.language === 'UZ')?.title || '', language: 'UZ' },
                    { title: game.translations.find(t => t.language === 'RU')?.title || '', language: 'RU' },
                    { title: game.translations.find(t => t.language === 'EN')?.title || '', language: 'EN' }
                ],
    ...
    const handleFileUpload = async (file: File) => {
        try {
            ...
            const response = await axios.post(`${baseUrl}/word-games/file`, uploadFormData, {
                headers: {
                            ...
                            {previewUrl ? (
                                <div className="mb-4">
                                    ...
                                    <p className="text-sm text-green-600 mt-2">
                                        {game.photo === formData.photo ? 'Joriy rasm' : 'Yangi rasm yuklandi'}
                                    </p>
